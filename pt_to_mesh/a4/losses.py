...
def eikonal_loss(gradients):
    ...
    # norm = torch.abs(torch.norm(gradients, dim=1) - torch.ones((gradients.shape[0],1)).to("cuda"))
    norm = (torch.norm(gradients, dim=1) - torch.ones((gradients.shape[0],1)).to("cuda"))**2
    return norm.mean()
