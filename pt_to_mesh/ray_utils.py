...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    xy_grid_sub_indices = torch.randperm(xy_grid.shape[0])
    xy_grid_sub = xy_grid[xy_grid_sub_indices].to("cuda")
    # Return
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    rays_d = camera.unproject_points(ndc_points, world_coordinates=True)
    # TODO (1.3): Get ray origins from camera center
    rays_o = camera.get_camera_center()
    rays_o = torch.repeat_interleave(rays_o, xy_grid.shape[0], dim=0)
    # assert(rays_o.shape == (65536, 3))
