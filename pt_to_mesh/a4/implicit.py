...
class NeuralSurface(torch.nn.Module):
    def __init__(
        ...
        layers = []
        layers.append(self.harmonic_embedding_distance)
        for layeri in range(cfg.n_layers_distance-1):
            if layeri == 0:
                dimin = embedding_dim_distance
                dimout = cfg.n_hidden_neurons_distance
            else:
                dimin = cfg.n_hidden_neurons_distance
                dimout = cfg.n_hidden_neurons_distance
        ...
        self.mlp = torch.nn.ModuleList(layers)
        self.mlp_dist = torch.nn.ModuleList(torch.nn.Sequential(
        ...
        for layeri in range(cfg.n_layers_color-1):
            dimin = cfg.n_hidden_neurons_color
            dimout = cfg.n_hidden_neurons_color

            linear = torch.nn.Linear(dimin, dimout)
            layers_color.append(torch.nn.Sequential(linear, torch.nn.ReLU()))
        layers_color.append(torch.nn.Sequential(torch.nn.Linear(cfg.n_hidden_neurons_color, 3), torch.nn.Sigmoid()))
        self.mlp_color = torch.nn.ModuleList(layers_color)
    ...
    def get_distance(
        ...
        for li, layer in enumerate(self.mlp):
            points = layer(points)
        for li, layer in enumerate(self.mlp_dist):
            points = layer(points)
        return points
    ...
    def get_color(
        ...
        for li, layer in enumerate(self.mlp):
            points = layer(points)
        for li, layer in enumerate(self.mlp_color):
            points = layer(points)
        return points
